...
import com.winfast.repository.EmailRepository;
import com.winfast.repository.SMSRepository;
...
public class MessageUtility {
	...
	public Response sendsms(String clientId, String network, String msisdn, String message, LocalDateTime requestIn) {
		...
		try {
			...
			return responseUtility.responseBuilder(smsObject.getUniqueId(), clientId, 0);

		} catch (Exception ex) {
			logger.error("sendsms :: " + ex.getMessage() + "\n" + ex.getLocalizedMessage() + "\n" + ex.getStackTrace());
			return responseUtility.responseBuilder(0L, clientId, 99);
		}
	...
	public Response sendMail(String clientId, String appId, String from, String to, String subject, String body, LocalDateTime requestIn) {
		...
		try {	
			...
			return responseUtility.responseBuilder(emailObject.getUniqueId(), clientId, 0);
			
		} catch (Exception ex) {
			logger.error(ex.getMessage() + "\n" + ex.getLocalizedMessage() + "\n" + ex.getStackTrace());
			return responseUtility.responseBuilder(emailObject.getUniqueId(), clientId, 99);
		}
	}
